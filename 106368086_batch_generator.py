...
class BatchGenerator:
    ...
    def __init__(self,
        ...
        if not filenames is None:
            if isinstance(filenames, (list, tuple)):
                ...
                with io.open(filenames, 'rb') as f:
        ...
        if not image_ids is None:
            if isinstance(image_ids, str):
                with io.open(image_ids, 'rb') as f:
    ...
    def parse_csv(self,
        ...
        with io.open(self.labels_filename, newline='') as csvfile:
    ...
    def parse_xml(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(self.images_dirs, self.image_set_filenames, self.annotations_dirs):
            ...
            with io.open(image_set_filename) as f:
            ...
            for image_id in image_ids:
                ...
                with io.open(os.path.join(annotations_dir, image_id+'.xml')) as f:
    ...
    def parse_json(self,
        ...
        with io.open(annotations_filenames[0], 'r') as f:
        ...
        for images_dir, annotations_filename in zip(self.images_dirs, self.annotations_filenames):
            # Load the JSON file.
            with io.open(annotations_filename, 'r') as f:
                annotations = json.load(f)
    ...
    def save_filenames_and_labels(self, filenames_path='filenames.pkl', labels_path=None, image_ids_path=None):
        ...
        '''
        with io.open(filenames_path, 'wb') as f:
            pickle.dump(self.filenames, f)
        if not labels_path is None:
            with io.open(labels_path, 'wb') as f:
                pickle.dump(self.labels, f)
        if not image_ids_path is None:
            with io.open(image_ids_path, 'wb') as f:
    ...
    def generate(self,
        ...
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                img_height = batch_X[i].shape[0]
                img_width = batch_X[i].shape[1]
    ...
    def process_offline(self,
        ...
        for k, filename in enumerate(self.filenames[start:stop]):
            i = k + start
            with Image.io.open('{}'.format(os.path.join(self.images_path, filename))) as img:
                image = np.array(img)
        ...
        with io.open('{}labels.csv'.format(dest_path), 'w', newline='') as csvfile:
